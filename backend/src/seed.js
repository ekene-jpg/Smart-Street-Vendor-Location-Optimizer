...
function run() {
  // INSERT OR IGNORE makes this safe to run every time the server starts
  // (e.g. on every restart/redeploy) even if the demo users already exist,
  // and safe against two instances starting at almost the same time.
  const hash = bcrypt.hashSync('Admin@123', 10);
  db.prepare(
    `INSERT OR IGNORE INTO User (full_name, email, password, role) VALUES (?, ?, ?, ?)`
...
try {
  ...
  // Never let a seeding hiccup take the whole API down — log it and continue,
  // since the app is still usable if seeding partially succeeded before.
  console.error('Seed step reported an issue (continuing to start the server):', err.message);
}
